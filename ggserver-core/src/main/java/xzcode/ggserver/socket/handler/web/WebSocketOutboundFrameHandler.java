...
import xzcode.ggserver.socket.executor.SocketRunnableTask;
...
public class WebSocketOutboundFrameHandler extends ChannelOutboundHandlerAdapter {
	...
	public WebSocketOutboundFrameHandler(GGSocketConfig config) {
		super();
		this.config = config;
	}
				...
				channelFuture.addListener(future -> {
					if (future.isSuccess()) {
						if (sendModel.getCallback() != null) {
							config.getTaskExecutor().submit(new SocketRunnableTask(sendModel.getCallback(), channel.attr(DefaultChannelAttributeKeys.SESSION).get()));
	    				}
					}
				});
			}
