package xzcode.ggserver.socket.handler.web;
...
import xzcode.ggserver.socket.message.SocketRequestTask;
...
public class WebSocketServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    public WebSocketServerHandler(GGSocketConfig config) {
		super();
		this.config = config;
	}
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
    	...
    	if (frame instanceof BinaryWebSocketFrame) {
            ...
            if (content.readableBytes() == 0) {            	
            	...
            	config.getTaskExecutor().submit(new SocketRequestTask(tag, ctx.channel().attr(DefaultChannelAttributeKeys.SESSION).get(), null ,config.getMessageInvokerManager(),config.getMessageFilterManager()));
          ...
          //如果有数据体
            ...
            //反序列化 并且 提交任务
            config.getTaskExecutor().submit(new SocketRequestTask(tag, ctx.channel().attr(DefaultChannelAttributeKeys.SESSION).get(), message, config.getMessageInvokerManager(),config.getMessageFilterManager()));
