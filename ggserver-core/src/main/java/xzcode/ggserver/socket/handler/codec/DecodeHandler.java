package xzcode.ggserver.socket.handler.codec;
...
import xzcode.ggserver.socket.serializer.ISerializer;
...
public class DecodeHandler extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		...
		if (invoker != null) {
			executor.submit(new SocketRequestTask(reqTag, ctx.channel().attr(DefaultChannelAttributeKeys.SESSION).get(),serializer.deserialize(data, invoker.getRequestMessageClass()),this.messageInvokerManager, this.messageFilterManager));
		}
