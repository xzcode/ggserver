            ...
            switch (((IdleStateEvent) evt).state()) {
				...
				case WRITER_IDLE:
						if(writerIdleEnabled) {
							...
							config.getTaskExecutor().submit(new GGEventTask(session, GGEvents.IdleState.WRITER_IDLE, config.getEventInvokerManager()));
						}
					break;
				case READER_IDLE:
						if (readerIdleEnabled) {
							...
							config.getTaskExecutor().submit(new GGEventTask(session, GGEvents.IdleState.READER_IDLE, config.getEventInvokerManager()));
						}
					break;
				case ALL_IDLE:
						if (allIdleEnabled) {
							...
							config.getTaskExecutor().submit(new GGEventTask(session, GGEvents.IdleState.ALL_IDLE, config.getEventInvokerManager()));
						}
					break;
				default:
					break;
