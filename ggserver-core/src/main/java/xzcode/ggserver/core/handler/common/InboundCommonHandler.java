...
public class InboundCommonHandler extends ChannelInboundHandlerAdapter{
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		config.getTaskExecutor().submit(new GGEventTask(session, GGEvents.ConnectionState.ACTIVE, config.getEventInvokerManager()));
		...
		ctx.channel().closeFuture().addListener((ChannelFuture future) -> {
			...
			config.getTaskExecutor().submit(new GGEventTask(sezzion, GGEvents.ConnectionState.CLOSE, config.getEventInvokerManager()));
