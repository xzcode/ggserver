package xzcode.ggserver.core.proxy.handler.web;
...
import xzcode.ggserver.core.proxy.channel.GGProxyChannelAttributeKeys;
import xzcode.ggserver.core.proxy.config.GGProxyConfig;
import xzcode.ggserver.core.proxy.message.receive.RequestMessageTask;
...
public class WebSocketInboundFrameHandler extends SimpleChannelInboundHandler<Object> {
    ...
    public WebSocketInboundFrameHandler(GGProxyConfig config) {
		this.config = config;
	}
    ...
    private void handleWebSocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) throws Exception {
    	...
    	if (frame instanceof BinaryWebSocketFrame) {
            ...
            if (content.readableBytes() == 0) {            	
            	...
            	config.getTaskExecutor().submit(new RequestMessageTask(tagBytes, null, ctx.channel().attr(GGProxyChannelAttributeKeys.SESSION).get(), config));
          ...
          //如果有数据体
            ...
            //提交任务
            config.getTaskExecutor().submit(new RequestMessageTask(tagBytes, bytes, ctx.channel().attr(GGProxyChannelAttributeKeys.SESSION).get(), config));
