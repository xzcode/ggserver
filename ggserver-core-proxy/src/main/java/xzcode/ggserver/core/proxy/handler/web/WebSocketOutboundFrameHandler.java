...
import xzcode.ggserver.core.proxy.executor.task.ExectionTask;
import xzcode.ggserver.core.proxy.message.send.SendModel;
...
public class WebSocketOutboundFrameHandler extends ChannelOutboundHandlerAdapter {
	...
	public WebSocketOutboundFrameHandler(GGProxyConfig config) {
		super();
		this.config = config;
	}
			...
			if (channelFuture != null) {
				if (sendModel.getCallback() != null) {
					channelFuture.addListener(future -> {
						if (future.isSuccess()) {
								config.getTaskExecutor().submit(new ExectionTask(sendModel.getCallback(), channel.attr(GGProxyChannelAttributeKeys.SESSION).get()));
						}
					});
				}
			}
