...
import xzcode.ggserver.core.proxy.event.GGEventTask;
import xzcode.ggserver.core.proxy.event.GGEvents;
...
public class InboundCommonHandler extends ChannelInboundHandlerAdapter{
	...
	public InboundCommonHandler(GGProxyConfig config) {
		super();
		this.config = config;
	}
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		ctx.channel().attr(GGProxyChannelAttributeKeys.SESSION).set(session);
		...
		ctx.channel().closeFuture().addListener((ChannelFuture future) -> {
			...
			GGSession sezzion = channel.attr(GGProxyChannelAttributeKeys.SESSION).getAndSet(null);
